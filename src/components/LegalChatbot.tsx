...
const BotAvatar = ({ size = "w-10 h-10" }: { size?: string }) => (
  <motion.img src={botAvatar} alt="น้องซื่อสัตย์" className={`${size} object-contain flex-shrink-0`}
    animate={{ rotate: [0, -3, 3, -3, 0] }} transition={{ repeat: Infinity, duration: 4, ease: "easeInOut" }} />
);
const BotAvatarSmall = () => (
  <motion.img src={botAvatar} alt="น้องซื่อสัตย์" className="w-10 h-10 object-contain flex-shrink-0 mt-0.5"
      ...
      <AnimatePresence>
        {!open && (
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }}
            ...
            <motion.img src={botAvatar} alt="น้องซื่อสัตย์" className="w-40 h-40 object-contain drop-shadow-xl"
              animate={{ y: [0, -8, 0] }} transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }}
              whileHover={{ scale: 1.25 }} whileTap={{ scale: 1.3 }} />
            <span className="bg-primary text-primary-foreground text-xs font-bold px-4 py-1.5 rounded-full shadow-md -mt-1">น้องซื่อสัตย์ AI</span>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {open && (
          <motion.div initial={{ opacity: 0, y: 20, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 20, scale: 0.95 }}
            ...
            <div className="relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-[#1e3a5f] via-[#2a5a8f] to-[#3a7ab5]" />
              <div className="relative p-4 flex items-center gap-3">
                <BotAvatar size="w-12 h-12" />
                <div className="flex-1 min-w-0">
                  <div className="font-heading font-bold text-white text-base">น้องซื่อสัตย์</div>
                  <div className="text-xs text-white/80">ผู้ช่วย AI ด้านกฎหมายไทย</div>
                  <div className="flex items-center gap-1.5 mt-0.5"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /><span className="text-xs text-green-200">ออนไลน์ — AWS Bedrock</span></div>
                </div>
                <button onClick={() => setOpen(false)} className="p-2 rounded-xl bg-white/10 hover:bg-white/20 transition-colors text-white" aria-label="ปิดแชท"><X className="w-5 h-5" /></button>
              </div>
            </div>
            ...
            <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
              {messages.length === 0 && (
                <div className="text-center py-4">
                  <BotAvatar size="w-20 h-20 mx-auto" />
                  <p className="text-sm font-bold mb-1 mt-3">สวัสดีครับ! ผม "น้องซื่อสัตย์" 🙏</p>
                  <p className="text-sm text-muted-foreground mb-4">กดเลือกหัวข้อ หรือพิมพ์คำถามได้เลย</p>
                  <div className="grid grid-cols-2 gap-2">
                    {QUICK_ACTIONS.map(a => (
                      <button key={a.label} onClick={() => sendMessage(a.label)}
                        className="bg-muted hover:bg-secondary border border-border rounded-xl p-3 text-left transition-colors group">
                        <span className="text-lg block mb-1">{a.emoji}</span>
                        <p className="text-xs font-medium leading-tight group-hover:text-primary">{a.label}</p>
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed ${
                    ...
                    {msg.role === "assistant" ? <div className="prose prose-sm max-w-none [&_p]:my-1 [&_ul]:my-1 [&_li]:my-0.5"><ReactMarkdown>{msg.content}</ReactMarkdown></div> : msg.content}
                  </div>
                  {msg.role === "user" && <div className="w-7 h-7 rounded-full bg-accent/20 flex items-center justify-center flex-shrink-0 mt-1"><User className="w-4 h-4 text-accent-foreground" /></div>}
                </div>
              ))}
              {isLoading && (
                <div className="flex gap-2"><BotAvatarSmall />
